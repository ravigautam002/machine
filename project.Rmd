...
Preprocessing is done through knnImpute, standardization and PCA. With PCA 80% of variance is set as the threshold.  
...
#PCA
preObj2 <- preProcess(training[,-53],method="pca",thresh=0.80)
training[,-53]<-predict(preObj2, training[,-53])
```
...
Test set is prepared and preprocessed in the same way as the training set. Test is further split to have 20% for model calibration and 20% for error estimate. The 4 models are calibrated against the test data. The one with highest accuracy is chosen which in this case is decision tree with bagging.  
...
testing[,-53]<-predict(preObj2, testing[,-53])
```
